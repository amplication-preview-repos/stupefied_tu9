/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PerformanceReportWhereInput } from "./PerformanceReportWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class PerformanceReportListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => PerformanceReportWhereInput,
  })
  @ValidateNested()
  @Type(() => PerformanceReportWhereInput)
  @IsOptional()
  @Field(() => PerformanceReportWhereInput, {
    nullable: true,
  })
  every?: PerformanceReportWhereInput;

  @ApiProperty({
    required: false,
    type: () => PerformanceReportWhereInput,
  })
  @ValidateNested()
  @Type(() => PerformanceReportWhereInput)
  @IsOptional()
  @Field(() => PerformanceReportWhereInput, {
    nullable: true,
  })
  some?: PerformanceReportWhereInput;

  @ApiProperty({
    required: false,
    type: () => PerformanceReportWhereInput,
  })
  @ValidateNested()
  @Type(() => PerformanceReportWhereInput)
  @IsOptional()
  @Field(() => PerformanceReportWhereInput, {
    nullable: true,
  })
  none?: PerformanceReportWhereInput;
}
export { PerformanceReportListRelationFilter as PerformanceReportListRelationFilter };
